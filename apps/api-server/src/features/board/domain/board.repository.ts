...
export type BoardRepository = {
    ...
    createPost(request: CreatePostRequest, user: CompletedUserRecord): Promise<PostEntity>;
    savePost(post: PostEntity, request: UpdatePostRequest): Promise<PostEntity>;
    deletePostWithComments(post: PostEntity): Promise<void>;
    createComment(postId: number, request: CreateCommentRequest, user: CompletedUserRecord): Promise<CommentEntity>;
