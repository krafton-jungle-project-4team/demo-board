...
```bash
if [ -n "${VERCEL_TOKEN:-}" ]; then echo "VERCEL_TOKEN is set"; fi
```

If this confirms the token is set, you're ready. Skip to Step 2. Do not print
the token value.

### B) Token is in a `.env` file under `VERCEL_TOKEN`

```bash
grep -q '^VERCEL_TOKEN=' .env 2>/dev/null && echo "VERCEL_TOKEN found in .env"
```
...
```bash
grep -i 'vercel' .env 2>/dev/null | sed 's/=.*$/=<redacted>/'
```
...
If none of the above yield a token, ask the user to set `VERCEL_TOKEN` in the
environment or add it to an ignored local `.env` file. Do not ask the user to
paste secret values into chat. They can create a Vercel access token at
...
```bash
# Check environment without printing values
[ -n "${VERCEL_PROJECT_ID:-}" ] && echo "VERCEL_PROJECT_ID is set"
[ -n "${VERCEL_ORG_ID:-}" ] && echo "VERCEL_ORG_ID is set"
...
- **Check whether tokens exist before asking the user.** Look in the current env and `.env` files first, but never print token values.
...
```bash
env | sed -n 's/^\(VERCEL_[A-Z0-9_]*\)=.*/\1=<set>/p'
grep -i vercel .env 2>/dev/null | sed 's/=.*$/=<redacted>/'
```
