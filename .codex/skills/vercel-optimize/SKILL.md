...
The bundled scripts live in this skill directory, not in the user's project.
When running commands from a project cwd, resolve the installed skill directory
first and prefix every bundled script path:

```bash
SKILL_DIR="<path-to-this-skill-directory>"
node "$SKILL_DIR/scripts/collect-signals.mjs" --help
```

All `node scripts/...` examples below are relative to `SKILL_DIR`.
